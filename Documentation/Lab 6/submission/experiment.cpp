...
RacecarExperiment() {
   ...
   // Start a subscriber to listen to mux messages
    ...
    // Skip the very first callback if dt is invalid
    if (dt_sec <= 0.0 || last_vesc_state.header.stamp.toSec() == 0.0) {
        return;
    }

    // Use IMU yaw with initial offset removed
    yaw = imu_yaw - init_yaw;

    // Euler integration for x and y
    x_odom += current_speed * std::cos(yaw) * dt_sec;
    y_odom += current_speed * std::sin(yaw) * dt_sec;
